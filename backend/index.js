...
app.get("/addHoldings", async (req, res) => {
  const existing = await HoldingsModel.countDocuments();
  if (existing > 0) {
    return res.send("Holdings already seeded.");
  }
...
app.get("/addPositions", async (req, res) => {
  const existing = await PositionsModel.countDocuments();
  if (existing > 0) {
    return res.send("Positions already seeded.");
  }
